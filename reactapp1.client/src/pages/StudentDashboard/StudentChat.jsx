...
import "./StudentDashboard.css"; // Reuse styling variables
...
export default function StudentChat() {
  ...
  useEffect(() => {
    authFetch("/student/discovery/my-mentors")
      .then(data => {
        const arr = Array.isArray(data) ? data : [];
        setMentors(arr);
        if (arr.length > 0) setActiveMentor(arr[0].mentorId);
      })
      .catch(() => {})
  ...
  const handleSend = async (e) => {
    e.preventDefault();
    if (!text.trim() || !activeMentor || sending) return;
    
    setSending(true);
    try {
      const newMsg = await authFetch(`/chat/${activeMentor}`, {
        method: "POST",
  ...
  return (
    <div style={{ display: "flex", height: "calc(100vh - 40px)", background: "var(--bg-card)", borderRadius: 20, overflow: "hidden", border: "1px solid var(--border)", margin: 20 }}>
      ...
      <div style={{ width: 280, borderRight: "1px solid var(--border)", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto" }}>
          {mentors.length === 0 ? (
            <div style={{ padding: 20, fontSize: 13, color: "var(--text-muted)" }}>You have no enrolled mentors yet. Request one in Discovery!</div>
          ) : (
            mentors.map(m => (
              <div 
                key={m.mentorId}
                onClick={() => setActiveMentor(m.mentorId)}
                style={{
                  padding: "16px 20px", display: "flex", alignItems: "center", gap: 12, cursor: "pointer",
                  background: activeMentor === m.mentorId ? "var(--accent-soft)" : "transparent",
                  borderLeft: activeMentor === m.mentorId ? "3px solid var(--accent)" : "3px solid transparent",
                  transition: "all 0.2s"
                }}
              >
      ...
      {activeMentor ? (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", background: "var(--bg-base)" }}>
          ...
          <div style={{ padding: "20px", borderBottom: "1px solid var(--border)", background: "var(--bg-card)" }}>
            <div style={{ fontWeight: 600 }}>Chat with {mentors.find(m => m.mentorId === activeMentor)?.name}</div>
          </div>
          ...
          <div style={{ flex: 1, padding: 20, overflowY: "auto", display: "flex", flexDirection: "column", gap: 12 }}>
            {messages.length === 0 ? (
              ...
              messages.map(msg => {
                const isMine = msg.senderType === "Student";
                return (
